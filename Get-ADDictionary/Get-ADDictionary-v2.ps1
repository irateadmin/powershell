...
foreach ($manager in $managers) 
{
    ...
    foreach($directreport in $directreports)
    {
        ...
        $csvfile = $outfile + "\$($manager.Name).csv" 
        foreach ($group in $groups)
        {
            @{
                    ...
                    Groups = $group.Name
                    GroupDescription = (@(get-adgroup $group -Properties description).Description)
            } |Select-Object Manager, DirectReport, Groups, GroupDescription | Export-Csv $csvfile -NoTypeInformation -Append
        }
    }
}
